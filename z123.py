...
class CEFRManager:
    def __init__(self, path="data.csv"):
        ...
        # 记录每个级别的总样本数（行数）
        self.level_sample_counts = self.df['cefr_level'].value_counts().to_dict()
        self.total_samples = sum(self.level_sample_counts.values())
    ...
    def build_statistics(self):
        ...
        level_weights = np.array([
            self.total_samples / (self.level_sample_counts.get(lvl, 1e-9)) 
            for lvl in CEFR_LEVELS
        ])

        for word in all_words:
            raw_freqs = np.array([self.word_cefr_freq[lvl][word] for lvl in CEFR_LEVELS], dtype=float)
            
            # 修正频次 (Actual Balanced Frequency)
            act_freqs = raw_freqs * level_weights
            
            total_act = act_freqs.sum()
            if total_act == 0:
                self.word_weighted_scores[word] = 0.0
            else:
                pcts = act_freqs / total_act
                weights = self.power_normalize(pcts)
                raw_score = sum(weights[i] * CEFR_VALUES[i] for i in range(len(CEFR_LEVELS)))
                self.word_weighted_scores[word] = raw_score

    def check(self, word):
        """DEBUG模式：对比展示原始频次与平衡后的实际频次"""
        word = word.lower()
        raw_freqs = np.array([self.word_cefr_freq[lvl][word] for lvl in CEFR_LEVELS], dtype=float)
        
        print(f"\n{'='*20} Balanced Analysis: '{word}' {'='*20}")
        if raw_freqs.sum() == 0:
            print(f"Result: Word '{word}' not found.")
            return

        level_weights = np.array([self.total_samples / self.level_sample_counts.get(lvl, 1e-9) for lvl in CEFR_LEVELS])
        act_freqs = raw_freqs * level_weights
        pcts = act_freqs / act_freqs.sum()
        weights = self.power_normalize(pcts)
        
        score = self.word_weighted_scores.get(word, 0.0)
        
        # 表格输出
        print(f"{'Level':<6} | {'Raw Freq':<10} | {'Act Freq':<12} | {'Bal-Pct %':<10} | {'Power W'}")
        print("-" * 65)
        for i, lvl in enumerate(CEFR_LEVELS):
            print(f"{lvl:<6} | {int(raw_freqs[i]):<10} | {act_freqs[i]:<12.2f} | {pcts[i]*100:<10.2f}% | {weights[i]:.4f}")
        print("-" * 65)
